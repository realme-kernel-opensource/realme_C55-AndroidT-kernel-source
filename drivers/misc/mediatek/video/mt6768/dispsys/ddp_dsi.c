...
/*
static int ddp_dsi_bdg_tx_phy_change(enum DISP_MODULE_ENUM module, void *handle,
	int en)
{
	unsigned char settxphy1[] = {0x11, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00}; //ID 0x10 ~ 1C
	unsigned char settxphy0[] = {0x11, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00}; //ID 0x
	...
	if (_is_power_on_status(DISP_MODULE_DSI0)) {
		...
		/* 2.wait mutex0_stream_eof: only used for video mode */
			...
			if (bdg_is_bdg_connected() == 1) {
				...
				//ddp_dsi_bdg_tx_phy_change(DISP_MODULE_DSI0, handle, en);
